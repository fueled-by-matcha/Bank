...
public class CheckingAccount extends Account{
	...
	*/
	...
	/*Method withdraws money from Account and takes into account overdraft fees depending on Customer type
	*/
	public double withdrawal(double amount){
		//check if there's need to reallocate
		if(index == capacity){
			reallocate();
		}
		transactions[index] = new Transaction((byte)1, 0, amount, "Withdrawal");
		index++;
		balance = balance - amount - customer.getCheckCharge();
	...
	*/
	public double addInterest(){
		double interest = balance * customer.getCheckInterest();
		if(index == capacity){
			reallocate();
		}
